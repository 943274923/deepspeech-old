...
QURAN_FILE = path.join(os.getcwd(), 'data/tarteel/quran.json')
...
class Quran:
    ...
    def __init__(self, quran_file: str = QURAN_FILE):
        self._quran_json = file_helper.open_json(quran_file)
    ...
    def surah(self, surah_number: int) -> Union[dict, None]:
        if (surah_number > MAX_SURAH_NUMBER) or (surah_number < MIN_SURAH_NUMBER):
            print('Surah number out of bounds')
            return None
        try:
            surah = self._quran_json[str(surah_number)]
            return surah
        except KeyError:
            print('Surah number not found')
            return None
